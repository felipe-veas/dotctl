...
func newInitCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "init",
		Short: "Initialize dotctl: configure and clone repo",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg.Repo.URL != "" && !flagForce {
				return fmt.Errorf("repo already configured; use --force to update it")
			}
			...
			if err := cfg.SetRepo(config.RepoConfig{
				URL:  repoURL,
				Path: repoPath,
			}); err != nil {
				return err
...
func resolveConfig() (*config.Config, string, error) {
	...
	if err != nil {
		if errors.Is(err, config.ErrNotFound) {
			return nil, cfgPath, fmt.Errorf("dotctl not initialized — run: dotctl init --repo <url>")
		}
		return nil, cfgPath, err
	}

	verbosef("config: path=%s repo=%s", cfgPath, cfg.Repo.Path)
	logging.Debug(
		"resolved config",
		"path", cfgPath,
